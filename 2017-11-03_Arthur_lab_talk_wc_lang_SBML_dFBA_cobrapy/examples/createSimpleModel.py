...
def check(value, message):
  # If 'value' indicates an error then print 'message' and exit
  if value == None:
    raise SystemExit('LibSBML returned Null while ' + message)
...
def create_model():
  # Example returns a simple but complete SBML Level 3 model
  try:
    document = SBMLDocument(3, 1)
  except ValueError:
    raise SystemExit('Could not create SBMLDocumention object')

  # Create a Model in the SBMLDocument object and add some units
